var app = angular.module('blockusApp', [])
  .controller('GameController', ['$scope', 'backendService', function ($scope, backendService) {
    ...
    $scope.doMove = function (x, y) {
      ...
      backendService.doMove($scope.gameId, $scope.playerId, $scope.selected.bid, $scope.selected.orientation_id, x, y)
      console.log("moved")

    }

    $scope.move_allowed = function (x, y) {
    ...
    $scope.positionBlocks = function () {
      ...
      $scope.blocks.forEach(function (element, index) {
        ...
        group.onMouseUp = function () {
          ...
          $scope.move_allowed(x, y);
          sleep(80);
          if ($scope.allowed_move) {
            $scope.doMove(x, y)
            this.locked = true;
            console.log('allowed')
